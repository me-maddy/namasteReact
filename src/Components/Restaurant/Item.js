const Item = ({ properties, hide }) => {
  const { name, description, price, isVeg, imageId } = properties;
  return (
    <div className="">
      <div className="top flex justify-between gap-x-5 w-full pb-5">
        <div className="text-gray-700 font-medium text-base w-10/12">
          ...
          <h6 className=" mb-4">&#8377;{price / 100}</h6>
        ...
        <div className="relative h-32 w-40">
          ...
          <div className="bg-white text-green-600 font-semibold px-6 py-2 cursor-pointer text-lg w-fit rounded-md absolute bottom-0 left-1/2 transition-shadow duration-200 ease-in -translate-x-1/2 translate-y-1/4 shadow-xl hover:shadow-2xl border border-solid border-gray-100">
            ADD
          </div>
        </div>
      </div>
