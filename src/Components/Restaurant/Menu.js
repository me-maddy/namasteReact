  ...
  return data.menu.length > 0 ? (
    <div className="w-full h-full mb-8 mt-16">
      <div className="w-10/12 sm:w-8/12 mx-auto">
        <div>
          <div className="flex justify-between items-center">
            ...
            <div className="rounded-md border border-gray-200 border-solid py-1.5 px-2.5 shadow-md">
              {/* <h3>
                  <StarOutlined className="icon" /> {data.mainData.avgRating}
                </h3> */}
        ...
        <hr className="border-y-8 text-gray-300 mt-12 mb-5 drop-shadow-sm" />
        {data?.menu?.map((restau, index) => (
          <Items
            key={restau?.card?.card?.title}
            info={restau?.card?.card}
